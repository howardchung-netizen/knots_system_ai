...
import { PDFCompare } from '../apollo/mutations';
...
const PDFCompareResult = ({ sourcePageVersionId, targetPageVersionId, open, handleClose }) => {
  ...
  const [pdfCompare, { data: pdfCompareData, loading: pdfCompareLoading }] = useMutation(PDFCompare, {
    //refetchQueries: [''],
    onCompleted: (data) => {
      const userErrors = data?.pdfCompare?.userErrors;
      ...
      setCompareImg(data?.pdfCompare?.pdfCompare?.fileUrl);
      setSourceImg(data?.pdfCompare?.pdfCompare?.sourcePageVersion?.fileUrl);
      setTargetImg(data?.pdfCompare?.pdfCompare?.targetPageVersion?.fileUrl);
    }
  });
  ...
  useEffect(() => {
    ...
    //   },
    // });
    if (sourcePageVersionId && targetPageVersionId) {
      pdfCompare({
        variables: {
          data: {
  ...
  }, [sourcePageVersionId, targetPageVersionId]);
      ...
      {activeButton === "page3" && (
        <div className='PDFCompare_result_content'>
          ...
          <div className='PDFCompare_result_slider_content'>
            <ReactCompareImage style={{height:'90%'}} leftImage={sourceImg} rightImage={targetImg}  />
          </div>
        </div>
      )}
      {activeButton === "page4" && (
