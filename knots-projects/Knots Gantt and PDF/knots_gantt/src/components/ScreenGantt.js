...
const ScreenGantt = props => {
  ...
  const projectConfig = {
    ...
    transport: {
      load: {
        url: `${props.REACT_APP_TODO_HTTP_ENDPOINT}/gantt-chart/load`,
        params: {
      ...
      sync: {
        url: `${props.REACT_APP_TODO_HTTP_ENDPOINT}/gantt-chart/sync`,
        params: {
          project: params.projectId,
  ...
  const headerTpl = (projectName = props.projectName) => `
    <div class="b-widget b-grid-header b-grid-header-text"
      style="justify-content:space-between;flex-direction:row;padding:8px 12px;height:45;border-bottom:1px solid #f3f3f3; align-items:center;">
        <div class="b-grid-header-text-content">
          <i class="b-fa b-fa-circle"></i>${props.projectName}
        </div>
        <div>
  ...
  const ganttConfig = {
    ...
    pdfExportFeature: {
      ...
      keepPathName: false,
      headerTpl,
      fileName: `${props.projectName}-${moment().format('YYYY-MM-DD')}`,
    },
    ...
    columns: [
      ...
      { type: 'startdate', text: 'Start', format: 'DD/M', width: 100 },
      { type: 'enddate', text: 'Due', width: 100,
        renderer: ({ record }) => {
          return Number(record.duration) !== 0 ? `${moment(record.endDate).subtract(1, "days").format("DD/M")}`
          : `${moment(record.endDate).format("DD/M")}`
        }
      },
      {
      ...
      { type: 'constraintdate', text: 'Constrain Date', format: 'DD/M' },
    ],
    ...
    async onBeforeLoadPage({ source }) {
      console.log('onBeforeLoadPage', source);
    },

    async onBeforeTaskEdit({ source, taskRecord, editor }) {
      taskRecord.moveDay = 0;
    },

    async onBeforeTaskSave({ source, taskRecord, editor }) {
      function updateTaskAndSubtasks(task, moveDay) {
        task.startDate = new Date(task.startDate);
        task.startDate.setDate(task.startDate.getDate() + moveDay);

        task.endDate = new Date(task.endDate);
        task.endDate.setDate(task.endDate.getDate() + moveDay);

        if (task.children) {
          task.children.forEach(childTask => {
            updateTaskAndSubtasks(childTask, moveDay);
          });
        }
      }

      if (taskRecord.moveDay) {
        updateTaskAndSubtasks(taskRecord, taskRecord.moveDay);
        await this.project.propagateAsync();
      }
    },
          ...
          generalTab: {
            items: {
              moveDayColumn: {
                type: 'numberField',
                name: 'moveDay',
                label: '+ or - Days (Include sub-tasks)',
                cls: 'b-inline',
                flex: '0.5',
              },
            },
