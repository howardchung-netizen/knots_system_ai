...
import { MEASUREMENT_SORT, MEASUREMENT_UPDATE } from "../apollo/mutations";
import { useSnackbar } from "notistack";
import MeasurementFormModal from "../components/measurement/MeasurementFormModal";
...
export default ({height}) => {
  ...
  const [dataUseQuery, queryStatus] = useLazyQuery(gql`${MEASUREMENT_QUERY} ${measurementFragment}`, {fetchPolicy: 'network-only',});
  const [formDataUpdateMutate, updateStatus] = useMutation(MEASUREMENT_UPDATE);
  const [formDataSortMutate, sortStatus] = useMutation(MEASUREMENT_SORT);
  ...
  const reorderRow = (draggedRowIndex, targetRowIndex) => {
    ...
    formDataSortMutate({
      ...
      onCompleted: (res) => {
        if (res.measurementSort.userErrors.length) {
          res.measurementSort.userErrors.map(e => {
            enqueueSnackbar(e.message, {
              variant: 'error'
            })
          })
        }
        else if (res.measurementSort.result) {
          queryStatus.refetch();
  ...
  const columns = React.useMemo(() => {
    let columns = [
      ...
      columnHelper.accessor('type', {
        ...
        cell: info => {
          return (
							info.row.original.type.nameCht+ ' | ' +info.row.original.type.nameEn
          )
        }
      }),
  ...
  const rows = useMemo(()=>{
    let rows = [];
    if(queryStatus.data?.measurements.edges) {
      rows = queryStatus.data?.measurements.edges.map((e)=>e.node);
      setTotalCount(queryStatus.data?.measurements.totalCount);
      setData(rows);
    }
    return rows
  ...
  return (
    <div style={{ height: height??'85vh', width: '100%', padding: 0 }}>
      <MeasurementFormModal
        ...
        onCompleted={onCreateCompleted}
      />
      <div style={{padding: 5, fontWeight: 'bold'}}>度量/單位列表</div>
      <Divider/>
			<FilterBlock>
