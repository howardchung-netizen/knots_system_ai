...
export default () => {
  ...
  const [showRatioDiscount, setShowRatioDiscount] = React.useState(false);
        ...
        if (qItems) {
          const listQuotationItem = (qItems, index) => {
            qItems.forEach((e, i) => {
              ...
              if(item.progress) sessionPrice += Math.ceil(amount * item.progress / 100);
              else sessionPrice += amount;
  ...
  const [termFontSize, setTermFontSize] = React.useState(defaultTermFontSize);
  const [itemFontSize, setItemFontSize] = React.useState(defaultItemFontSize);
  ...
  const [lowerNameFontSize, setLowerNameFontSize] = React.useState(itemFontSize - 1);
  const [upperDescFontSize, setUpperDescFontSize] = React.useState(itemFontSize - 2);
  const [lowerDescFontSize, setLowerDescFontSize] = React.useState(itemFontSize - 2);
  ...
  const DivHeight = ({ index, name, desc, price, qty, unit, amount, upper, isHideNo, isSession, progress, child }) => {
    let isUpper = upper == 0;
        ...
        <table style={{ width: '210mm', borderTop: '1px solid black', borderCollapse: 'collapse', fontSize: itemFontSize }}>
          <tbody>
            <tr style={{ fontWeight: 'bold', height: isSession ? sessionHeight : 'auto', fontFamily: fontFamily, fontSize: itemFontSize }}>
              <td style={{ ...baseTdStyle, verticalAlign: 'middle', backgroundColor: backgroundColor, width: '5%' }}>{isHideNo ? '' : index}</td>
              <td style={{ borderRight: '1px solid black', borderCollapse: 'collapse', paddingLeft: 5, paddingTop: rowPaddingTop, paddingBottom: rowPaddingBottom, backgroundColor: backgroundColor, width: contentMaxWidth }}>
                <div style={{ width: contentMaxWidth }}>{!isUpper && !isSession ? ' - ' : ''}{name}</div>
              ...
              <td style={{ borderRight: '1px solid black', borderCollapse: 'collapse', textAlign: 'center', backgroundColor: backgroundColor, verticalAlign: 'middle', width: '13%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: 3, paddingRight: 3 }}>
                  <div>{showProgress && progress ? progress + '%' : ''}</div>
                  <div>
                    <div>
                      {amount && progress ? toMoney(Math.ceil(amount * progress / 100)) : ''}
                      {amount && !progress? toMoney(amount) : ''}
                    </div>
                  </div>
                </div>
              </td>
              ...
              <table width="100%" style={{ tableLayout: 'fixed', marginBottom: 0 }}>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className='' style={{ textAlign: 'right' }} colSpan="12%">
                      Invoice No :
                    </td>
            ...
            <tfoot colSpan={5} style={{ fontWeight: 'bold' }}>
              ...
              }
              {
                quotation?.ratioDiscount > 0 && <tr style={{ height: 'auto', borderTop: '1px solid black' }}>
                  <ItemPageSummeryTd
                    title={`Discount Off:`}
                    desc={`${showRatioDiscount ? `${quotation.discountRatio ?? 0}%` : ''}`}
  ...
  const Item = ({ index, name, desc, price, qty, unit, amount, upper, isHideNo, isSession, progress, child }) => {
    let isUpper = upper == 0;
    let backgroundColor = null;
    if(isSession && showSession) backgroundColor = '#ebebeb';
    else if(isUpper) backgroundColor = '#bcd7f3';
    ...
    return (
      <>
      <tr style={{ fontWeight: 'bold', height: isSession ? sessionHeight : 'auto', fontFamily: fontFamily, fontSize: itemFontSize}}>
        <td style={{...baseTdStyle, verticalAlign: 'middle', backgroundColor: backgroundColor}}>{isHideNo ? '' : index}</td>
        <td style={{ borderRight: '1px solid black', borderCollapse: 'collapse', paddingLeft: 5, paddingTop: rowPaddingTop , paddingBottom: rowPaddingBottom, backgroundColor: backgroundColor }}>
          <div style={{width: contentMaxWidth}}>{!isUpper && !isSession ? ' - ' : ''}{name}</div>
          {showItemDesc && desc?.length > 1 && <div style={{ fontSize: isUpper ? upperDescFontSize : lowerDescFontSize, fontWeight: 300, width: contentMaxWidth }}>{showItemDesc ? desc : ''}</div>}
        </td>
        { 
  ...
  }, [itemFontSize, termFontSize])

  useEffect(()=>{
    query();
  },[lang, showItemDesc, showPriceAndUnit, showUnitPrice, showSignature, showSession])
        ...
        <div className="print-view">
                ...
                <div style={{ display: 'flex' }}><div style={{ width: 100 }}><strong>顯示:</strong></div>
                  <div>
                    <FormGroup row>
                      ...
                      <FormControlLabel control={<Checkbox checked={showProgress} onChange={(e) => { setShowProgress(e.target.checked) }} />} label="進度" />
                      <FormControlLabel control={<Checkbox checked={showRatioDiscount} onChange={(e) => { setShowRatioDiscount(e.target.checked) }} />} label="折扣%" />
                    </FormGroup>
                  </div>
                </div>
          ...
          <ComponentToPrint ref={componentRef}>
            ...
            >
              ...
              {
                pages.length > 0 && (pages[pages.length - 1][pages[pages.length - 1].length - 1].sumOfRowHeight + signtureBlockHeight > itemListMaxHeight) && <Page index={pages.length + 1} totalPage={pages.length + 1}><SignatureBlock /></Page>
              }
            </div>
          </ComponentToPrint>
        </div>
