...
import { PORJECT_TYPES_QUERY } from "../apollo/queries";
import { projectTypeFragment } from "../apollo/fragments";
import { DraggableTableRow, TableRow } from "../components/TableRow";
import { IOSSwitch } from "../components/MuiSwitch";
import ProjectTypeFormModal from "../components/project/ProjectTypeFormModal";
import { PROJECT_TYPE_SORT, PROJECT_TYPE_UPDATE } from "../apollo/mutations";
...
export default ({height}) => {
  ...
  const [dataUseQuery, queryStatus] = useLazyQuery(gql`${PORJECT_TYPES_QUERY} ${projectTypeFragment}`, {fetchPolicy: 'network-only',});
  const [formDataUpdateMutate, updateStatus] = useMutation(PROJECT_TYPE_UPDATE);
  const [formDataSortMutate, sortStatus] = useMutation(PROJECT_TYPE_SORT);
  ...
  const reorderRow = (draggedRowIndex, targetRowIndex) => {
    ...
    formDataSortMutate({
      ...
      onCompleted: (res) => {
        if (res.projectTypeSort.userErrors.length) {
          res.projectTypeSort.userErrors.map(e => {
            enqueueSnackbar(e.message, {
              variant: 'error'
            })
          })
        }
        else if (res.projectTypeSort.result) {
          queryStatus.refetch();
  ...
  const rows = useMemo(()=>{
    let rows = [];
    if(queryStatus.data?.projectTypes.edges) {
      rows = queryStatus.data?.projectTypes.edges.map((e)=>e.node);
      setTotalCount(queryStatus.data?.projectTypes.totalCount);
      setData(rows);
    }
    return rows
  ...
  const onDeleteBtnClick = (id) => {
    formDataUpdateMutate({
      ...
      onCompleted: (res) => {
        if (res.projectTypeUpdate.userErrors.length) {
          res.projectTypeUpdate.userErrors.map(e => {
            enqueueSnackbar(e.message, {
              variant: 'error'
            })
          })
        }
        else if (res.projectTypeUpdate.projectType) {
          queryStatus.refetch();
  ...
  return (
    <div style={{ height: height??'85vh', width: '100%', padding: 0 }}>
      <ProjectTypeFormModal
        ...
        // onCompleted={onCreateCompleted}
      />
      <div style={{padding: 5, fontWeight: 'bold'}}>工程類型列表</div>
      <Divider/>
			<FilterBlock>
