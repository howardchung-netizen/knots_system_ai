...
import { GET_PERMISSION, GET_RESOURCE } from '../../apollo/queries';
import { PERMISSION_CREATE, PERMISSION_UPDATE, PERMISSION_DELETE } from '../../apollo/mutations';
...
export default function PermissionTable(props) {
  ...
  const { data, error, loading, refetch } = useQuery(GET_PERMISSION, {
    fetchPolicy: 'network-only',
    notifyOnNetworkStatusChange: true,
    variables: {}
  });

  const { data: resourceData, loading: resourceLoading } = useQuery(GET_RESOURCE, {
    fetchPolicy: 'network-only',
    notifyOnNetworkStatusChange: true,
  });
  ...
  const resources = useMemo(() => {
    let resources = ['.*'];
    if (resourceData) resourceData.resources.map(e => resources.push(e));
    return resources
  }, [resourceData])

  const [rowData, setData] = React.useState([]);
  const rows = useMemo(() => {
    let rows = data?.permissions || [];
    setData(rows)
    return rows;
  }, [data])

  const [create, { loading: createLoading }] = useMutation(PERMISSION_CREATE, {
    onCompleted: (data) => {
      const userErrors = data?.permissionCreate?.userErrors;
  ...
  const [update, { loading: updateLoading }] = useMutation(PERMISSION_UPDATE, {
    onCompleted: (data) => {
      const userErrors = data?.permissionUpdate?.userErrors;
      if (userErrors.length) {
        userErrors.map(e => {
          enqueueSnackbar(e.message, {
            variant: 'error'
  ...
  const [del, { loading: deleteLoading }] = useMutation(PERMISSION_DELETE, {
    onCompleted: (data) => {
      const userErrors = data?.permissionDelete?.userErrors;
      if (userErrors.length) {
        userErrors.map(e => {
          enqueueSnackbar(e.message, {
            variant: 'error'
        ...
        cell: info => {
          return (
            <Stack
              ...
              {info.getValue().map(x => <Chip key={x} label={x} />)}
            </Stack>
          )
        }
      }),
  ...
  const permissionConfig = [{ name: 'GET', value: 'GET' }, { name: 'CREATE', value: 'CREATE' }
    , { name: 'UPDATE', value: 'UPDATE' }, { name: 'DELETE', value: 'DELETE' }, { name: 'ALL', value: 'ALL' }]
