...
import { QUOTATION_TEMPLATE_CREATE, QUOTATION_TEMPLATE_IMPORT_ITEM } from '../../apollo/mutations';
...
export default function ({ open, onCloseClick, onCompleted, ...props }) {
  ...
  const lang = queryParam.get("lang") || 'Cht';
  const { enqueueSnackbar } = useSnackbar();
  const [formData, setFormData] = React.useState({id: templateId});
  ...
  const [formDataUpdateMutate, updateStatus] = useMutation(QUOTATION_TEMPLATE_IMPORT_ITEM);
  ...
  const _onConfirmClick = () => {
    ...
    formDataUpdateMutate({
      ...
      onCompleted: (res) => {
        if (res.quotationTemplateImportItem.userErrors.length) {
          res.quotationTemplateImportItem.userErrors.map(e => {
            enqueueSnackbar(e.message, {
              variant: 'error'
            })
          })
        }
        else if (res.quotationTemplateImportItem.quotationTemplate) {
