...
import { BOOK_KEEPING_PERIOD_EXPENSE_CREATE, BOOK_KEEPING_PERIOD_EXPENSE_UPDATE } from '../../apollo/mutations';
...
export default function ({ open, onCloseClick, onCompleted, ...props }) {
  ...
  const [formDataCreateMutate, createStatus] = useMutation(BOOK_KEEPING_PERIOD_EXPENSE_CREATE);
  const [formDataUpdateMutate, updateStatus] = useMutation(BOOK_KEEPING_PERIOD_EXPENSE_UPDATE);
  ...
  const CategoryAccountIdSelect = () => {
    let _bookKeepingAccountOptions = bookKeepingAccountOptions ? bookKeepingAccountOptions.filter(e => e.isPlaceholder == false).sort((a,b)=>{
      if(a.accountType?.name > b.accountType?.name) return 1;
      if(a.accountType?.name < b.accountType?.name) return -1;
      return 0;
    }).map(e=> {
      return {
        searchValue: e.name+e.accountType?.name,
        value: e.accountType.id,
        label: e.name,
        ...e
      }
    })  : [];
  ...
  const BookKeepingPeriodSelect = () => { 
    return (
      <Select
        ...
        onChange={(e) => {
          onFormDataChange(["period"], [e.target.value])
        }}
      />
    )
  }
              ...
              <Grid container spacing={2} padding={1}>
                ...
                <Grid item xs={'auto'} sx={{ minWidth: 100, maxWidth: 100 }}>
                  <Input
                    ...
                    onChange={(e) => { 
                      const periodDay = new Date(e.target.value).getDate();
                      onFormDataChange(["fromDate", "periodDay"], [e.target.value, periodDay])
                     }}
                  />
                </Grid>
                <Grid item xs={'auto'} sx={{ minWidth: 100, maxWidth: 100 }}>
