...
import { PROJECT_TYPE_CREATE, PROJECT_TYPE_UPDATE } from '../../apollo/mutations';
...
export default function ({ open, onCloseClick, onCompleted, ...props }) {
  ...
  const [formDataCreateMutate, createStatus] = useMutation(PROJECT_TYPE_CREATE);
  ...
  const checkInputError = () => {
    let inputError = {};
    for (let i of ["nameCht", "nameEn", "code"]) {
  ...
  const _onConfirmClick = () => {
    ...
    if (props.mode == 'create') formDataCreateMutate({
      ...
      onCompleted: (res) => {
        if (res.projectTypeCreate.userErrors.length) {
          res.projectTypeCreate.userErrors.map(e => {
            enqueueSnackbar(e.message, {
              variant: 'error'
            })
          })
        }
        else if (res.projectTypeCreate.projectType) {
