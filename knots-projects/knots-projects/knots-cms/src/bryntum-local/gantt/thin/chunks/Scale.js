...
class Histogram extends Widget {
  ...
  set series(value) {
    ...
    for (const id in value) {
      ...
      if (value[id] !== false) {
        const data = series[id] = Object.assign({}, value[id]); // support type & field provided on config prototype level
  ...
  set data(data) {
    ...
    me._data = data; // Calculate the top value from all the series
    ...
    for (let index = 0, x = 0, {
      ...
      height = datum.height || (value > topValue ? topValue : value) / topValue,
        ...
        dataset: {}
            ...
            barTip = me.callback('getBarTip', me, [series, rectConfig, datum, index]); // use either provided width or the calculated value
  ...
  drawOutline(series) {
    ...
    for (let i = 0, x = 0, {
      ...
      const barHeight = 1 - data[i][field] / topValue; // use either provided with or the calculated value
  ...
  getBarTip(series, rectConfig, datum, index) {} // Injectable method

  getBarTextTip(renderData, datum, index) {} // Injectable method
