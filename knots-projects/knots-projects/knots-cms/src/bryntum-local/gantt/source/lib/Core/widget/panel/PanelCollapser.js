...
export default class PanelCollapser extends Base.mixin(Factoryable) {
    ...
    lockInnerSize(lock = true) {
        ...
        // flips to position:absolute... it will not longer be providing a natural size to prop up the panel so we
    ...
    updatePanel(panel) {
        ...
        me.panelListeners?.();
        me.panelListeners = (panel && !panel.isPainted)
            ? panel.on({
                once : true,
                paint() {
                    if (panel.initialConfig.collapsed) {
