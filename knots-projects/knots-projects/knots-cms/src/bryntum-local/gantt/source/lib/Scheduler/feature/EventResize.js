...
export default class EventResize extends InstancePlugin.mixin(Draggable, Droppable) {
    ...
    beforeDrag(drag) {
        ...
        if (this.disabled || client.readOnly || eventRecord?.readOnly || super.beforeDrag(drag) === false) {
            return false;
        }
    ...
    unHighlightHandle(item = this.overItem) {
        if (item) {
            const inner = item.syncIdMap?.[this.client.scheduledEventName] ?? item.querySelector(this.client.eventInnerSelector);
