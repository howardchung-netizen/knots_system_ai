...
export default class TaskEdit extends InstancePlugin.mixin(
    ...
    async save() {
        ...
        if (me.isEditing) {
            ...
            me.trigger('afterTaskSave', { taskRecord, editor });
    ...
    async cancel() {
        ...
        // Newly created records are already in TaskStore, must be removed on cancel
    ...
    async delete() {
        const
            me                                         = this,
            { editor, scheduler, record : taskRecord } = me;

        if (me.trigger('beforeTaskDelete', { taskRecord, editor }) === false) {
            return;
        }
        ...
        scheduler.trigger('afterTaskEdit', { editor });
    }
