 ...
 * @classType widget
 */
export default class Widget extends Base.mixin(Localizable, Events, Delayable, Identifiable, Factoryable, RTL) {
    //region Config
    ...
    doHideOrRealign({ target, isTrusted }) {
        ...
        if (lastTarget?.nodeType === Node.ELEMENT_NODE && !target.contains(lastTarget) && target.nodeType !== Element.DOCUMENT_NODE) {
            return;
        }
    ...
    updateRtl(rtl) {
        ...
        // Cascade the rtl setting to owned widgets
