...
export default class SchedulerBase extends TimelineBase.mixin(
    ...
    editEvent(eventRecord, resourceRecord, element) {
        ...
        if (eventRecord.eventStore !== eventStore) {
            ...
            if (me.trigger('beforeEventAdd', { eventRecord, resourceRecords }) === false) {
    ...
    async internalAddEvent(startDate, resourceRecord, row) {
        ...
        const assignments = assignmentStore?.assignEventToResource(eventRecord, resourceRecord);
    ...
    async scheduleEvent({ startDate, eventRecord, resourceRecord, element }) {
        ...
        if (element) {
            const eventRect = Rectangle.from(element, this.foregroundCanvas);
