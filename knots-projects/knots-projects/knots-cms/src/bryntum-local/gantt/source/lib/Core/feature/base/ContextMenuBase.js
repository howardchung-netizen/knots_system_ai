...
export default class ContextMenuBase extends InstancePlugin {
    ...
    onElementKeyDown(event) {
        if (!event.handled && event.key === ' ') {
            const
                { focusedCell } = this.client,
                { cell }        = focusedCell;

            if (!focusedCell.isActionable && this.getDataFromEvent(event)) {
                cell.dispatchEvent(this.createContextMenuEventForElement(cell));
            }
        }
    }
    ...
    internalShowContextMenu(event) {
        ...
        if (data && me.shouldShowMenu(data)) {
            ...
            if (me.beforeContextMenuShow(menuContext) !== false) {
                ...
                if (client.trigger(`${type}MenuBeforeShow`, menuContext) !== false) {
                    ...
                    if (alignSpec) {
                        ...
                        menu.showByPoint(menuContext.point);
                    }
    ...
    createContextMenuEventForElement(targetElement) {
        const
            // See if it's clipped by a scroller so that we center in the *visible* center,
            // Not the center of some long, overflowing element (eg a TimeAxisColumn header)
            scrollerElement  = targetElement.closest('.b-widget-scroller'),
            targetPoint      = Rectangle.from(targetElement.offsetWidth > scrollerElement?.clientWidth ? scrollerElement : targetElement).center,
