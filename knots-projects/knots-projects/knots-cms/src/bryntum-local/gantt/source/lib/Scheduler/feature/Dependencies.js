...
export default class Dependencies extends InstancePlugin.mixin(
    ...
    static get pluginConfig() {
        return {
            chain : ['render', 'onPaint', 'renderContents', 'onElementClick', 'onElementDblClick', 'onElementMouseOver',
    ...
    onProjectRefresh({ isInitialCommit, isCalculated }) {
        const
            me                         = this,
            { toDrawOnProjectRefresh } = me;
        ...
        if (!me.client?.refreshSuspended) {
            ...
            else if (toDrawOnProjectRefresh?.size > 0) {
    ...
    async onDependencyChange({ action, record, records }) {
        ...
        switch (action) {
            case 'dataset':
                me.resetGridCache();
                // Allow no records to fall through to clear existing deps
                if (records.length) {
     ...
     * @private
     */
    onEventChange({ action, record, records, changes, fromBatched }) {
    ...
    getBox(dependency, source, assignmentData = null, roughly = false) {
        ...
        const result = this.client.currentOrientation.getResourceEventBox(eventRecord, resourceRecord, true, roughly);
    ...
    getDependencyBounds(dependency, assignmentData, roughly = true) {
        ...
        const ddr = dependency.getDateRange();
        ...
        const
            ...
            { resourceMap } = client.currentOrientation,
            { bounds }      = instanceMeta;
        ...
        if (
            // Forcing bounds
            me._resetBoundsCache ||
            // No bounds
            !bounds ||
            // Start rect approximated and it can now be properly calculated
            (!bounds.startRectangle.layout && resourceMap?.has(bounds.startRectangle.resourceId)) ||
            // End rect approximated and it can now be properly calculated
            (!bounds.endRectangle.layout && resourceMap?.has(bounds.endRectangle.resourceId))
        ) {
            const
                    ...
                    if (client.milestoneLayoutMode === 'default' && record.isMilestone) {
                        ...
                        // use cached value for default milestones
                            ...
                            if (!client.milestoneWidth) {
                                const milestoneElement = client.getElementFromAssignmentRecord(assignment);
                                if (milestoneElement) {
                                    client.milestoneWidth = milestoneElement.offsetWidth;
    ...
    get dependencyGridCache() {
        ...
        // Spawn the grid cache on request, but not before project is ready for the first time.
    ...
    doResetBoundsCache() {
        const { dependencyGridCache } = this;

        // Iterate over the dependency grid cache and clear all cached bounds
        Object.values(dependencyGridCache).forEach(rowCache => {
            Object.values(rowCache).forEach(value => {
                value.forEach(({ dependency, metaId }) => {
    ...
    drawDependency(dependency, drawData = null, assignmentData = null, canvas = this.client.svgCanvas, cache = true) {
        ...
        if (!me.disabled && dependencyDrawData) {
            ...
            // added
            const { hideMarker } = dependencyDrawData.lineDef;

            me.drawLine(canvas, dependency, dependencyDrawData.lines, assignmentData, cache, hideMarker);
    ...
    draw(reLayout = false) {
        ...
        if (!me.disabled && !scheduler.suspendRendering &&
            ...
            const
                consideredDependencies   = new Set(),
                {
                    dependencyGridCache,
                    cacheGridSize
                }                        = me,
                ...
                for (rowIndex = topIndex; rowIndex <= bottomIndex; rowIndex++) {
                    ...
                    for (i = 0; deps && i < deps.length; i++) {
                        ...
                        if (!consideredDependencies.has(flagId)) {
                            ...
                            const bounds = me.getDependencyBounds(dependency, assignmentData, false);
