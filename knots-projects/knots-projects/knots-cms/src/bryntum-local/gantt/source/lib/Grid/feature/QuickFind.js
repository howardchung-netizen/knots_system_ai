...
export default class QuickFind extends InstancePlugin {
    ...
    onElementKeyDown(event) {
        const
            me              = this,
            { grid }        = me,
            filterFeature   = grid.features.filter,
            { focusedCell } = grid;

        // Only react to keystrokes on grid cell elements.
        if (!me.disabled && focusedCell?.record && !focusedCell.isActionable) {
            if (me.find.length > 0) {
                // backspace
                ...
                // Ctrl+Shift+F
                    ...
                    me.clear();
                    filterFeature.showFilterEditor(grid.columns.getById(me.columnId), me.find);
                }
            }
        }
    }
    ...
    onElementKeyPress(event) {
        ...
        // Only react to keystrokes on grid cell elements
        if (!me.disabled && focusedCell?.record && !focusedCell.isActionable && event.key !== 'Enter') {
