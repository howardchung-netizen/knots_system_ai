...
export default class Tooltip extends Popup {
    ...
    internalOnPointerOver(event) {
        ...
        if (forSelector) {
            ...
            if (activeTarget?.contains(event.target) && activeTarget.contains(event.relatedTarget)) {
                return;
            }
            newTarget = event.target.closest(forSelector);

            // Mouseovers while within a target do nothing
            if (newTarget && event.relatedTarget?.closest(forSelector) === newTarget) {
                return;
            }
        }
    ...
    updateContent() {
        ...
        if (me.getHtml) {
            // Ensure the html setter does not veto the operation as a no-op.
