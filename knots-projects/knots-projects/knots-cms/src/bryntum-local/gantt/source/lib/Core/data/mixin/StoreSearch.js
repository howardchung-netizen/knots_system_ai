...
const
    ...
    comparisons = {
        ...
        object    : (value, find) => value === find, // typeof null === object
        undefined : (value, find) => value === find
    };
...
export default Target => class StoreSearch extends (Target || Base) {
    ...
    search(find, fields = null) {
        ...
        for (i = 0; i < len; i++) {
            ...
            for (const key of fields || record.fieldNames) {
                ...
                if (value && comparison?.(value, find)) {
                    found.push({
                        index : i,
                        data  : record,
                        field : key,
     ...
     * @category Search
     */
    findByField(field, value, distinct = false) {
        const
            records    = this.storage.values,
            len        = records.length,
            usedValues = new Set(),
        ...
        for (i = 0; i < len; i++) {
            ...
            if (!distinct || !usedValues.has(fieldValue)) {
                const
                    ...
                    comparison = comparisons[type];

                if ((value == null && fieldValue === value) || value && comparison(fieldValue, value)) {
                    found.push({
                        id    : record.id,
                        index : i,
                        data  : record
