 ...
 * @module Core/Base
 */
import Objects, { typeOf } from './helper/util/Objects.js';
...
export default class Base {
    ...
    peekConfig(name) {
        ...
        if (config[name] != null || hasOwnProperty.call(config, name)) {
            return config[name];
        }
    }
    ...
    getConfigValue(name, options) {
        ...
        if (!this.$meta.configs[name]?.lazy) {
            return getValue(this[name]);
        }
        // Instead pull their initial config in
        else if (this[lazyConfigsSymbol]?.[name]) {
