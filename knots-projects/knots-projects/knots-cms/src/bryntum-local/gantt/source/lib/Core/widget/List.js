...
export default class List extends Widget {
    ...
    selectAllItemTpl() {
        const selectedCls = this.selected.equals(this.store.records) ? 'b-selected' : '';
    ...
    onStoreChange({ source : store, action, records, record, changes }) {
        ...
        switch (action) {
            // Our refresh listener will pick this up
            case 'dataset':
                return;
            case 'remove':
    ...
    updateSelectAllState() {
        const
            ...
            allRecordsSelected = me.selected.equals(me.store.records);

        selectAllEl?.classList.toggle(me.selectedCls, allRecordsSelected);
    }
